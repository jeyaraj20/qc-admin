...
const AddEditPassage = (props) => {
    ...
    let { maincategory, subcategory, toggle, categoryId, subcategoryId, qid, mode } = props;
    ...
    const handleRefresh = async () => {
        ...
        if (mode != 'Edit') {
            ...
            console.log(await questionService.getPassageQuestionById(qid));
    ...
    const mapRows = (rows) => {
        ...
        let quesrows = rows.map((obj, index) => {
            ...
            row.delete = (
                <IconButton onClick={() => deletePassageQuestion(obj.index)} className="icon-btn">
    ...
    */
    ...
    const saveQuestion = async () => {
        try {
            ...
            setPassageQuestionArr(passagequestionarr);
            setPassageForm(formdata);
            setIndex(index + 1);
        } catch (ex) {
    ...
    const savePassageQuestion = async () => {
        console.log(passagequestionarr);
        let savedata = {};
        savedata.cat_id = categoryId;
        savedata.sub_id = subcategoryId;
        savedata.q_type = 'P';
        if (window.getQuestionvalue() != '') {
            let questionvalue = window.getQuestionvalue();
            questionvalue = entities.encode(questionvalue);
            savedata.question = questionvalue;
        } else {
            setLoader('none');
            setUnLoader('block');
            setAlertMessage('Question is required');
        ...
        savedata.crt_ans = '';
        if (difficultyLevel != '') {
            savedata.quest_level = difficultyLevel;
        } else {
            setLoader('none');
            setUnLoader('block');
        ...
        savedata.quest_pos = 0;
        savedata.quest_ipaddr = ipaddr;
        savedata.passage_questions = passagequestionarr;

        await questionService.savePassageQuestion(savedata);
                ...
                <div style={{ padding: '1%' }} className="col-lg-12 d-flex flex-column order-lg-1">
                    <div className="row">
                        <div className="col-lg-12 col-sm-6 col-12">
                            <TextField label={'Question ID'} value={questionNo} margin="normal" />
                        </div>
                        <div className="col-lg-10 col-sm-10 col-10">
                            <h4 style={{ padding: '0.5%' }}>
                                Add New Passage for ({maincategory} - {subcategory})
                            </h4>
                        </div>
                    ...
                    <div className="col-lg-12 col-sm-6 col-12">
                        <h5>
                            Passage <span style={{ color: 'red' }}>*</span>
                        </h5>
                        <textarea
                            value={question}
                            onChange={() => window.getQuestionvalue()}
                            class="ckeditor form-control  validate[required]"
                            rows="4"
                    ...
                    >
                        <Button
                            variant="contained"
                            onClick={() => showAddPassageQuestion('')}
                            color="primary"
                    ...
                    <div id="jsforckeditor">{ckeditorflag && <>{loadCkeditorfuncion()}</>}</div>
                </div>
                            ...
                            <div className="col-lg-12 d-flex flex-column order-lg-1">
                                ...
                                >
                                    <div className="row">
                                        <div className="col-lg-10 col-sm-10 col-10">
                                            <h4 style={{ padding: '0.5%' }}>
                                                Add New Question for ({maincategory} - {subcategory})
                                            </h4>
                                        </div>
                                        <div className="col-lg-2 col-sm-2 col-2">
                                            <Button
                                    ...
                                    <div id="jsforckeditormodal">
                                        {ckeditorflag && <>{loadCkeditorfuncion()}</>}
                                    </div>
                                </div>
                            </div>
                        </div>
