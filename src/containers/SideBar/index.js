...
const SideBar = (props) => {
  ...
  const {toggle,navCollapsednew, drawerType, width, navigationStyle} = props;

  const [widthnow, setWidthnow] = useState(width);
  //const [navCollapsednow, setWidthnow] = useState(navCollapsed);
  

  useEffect(() => {
    window.addEventListener('resize', () => {
      //dispatch(updateWindowWidth(window.innerWidth))
      setWidthnow(window.innerWidth);
    });
  }, [dispatch]);
  ...
  let type = 'permanent';
  if (drawerType.includes(COLLAPSED_DRAWER) || (drawerType.includes(FIXED_DRAWER) && widthnow < 1200)) {
    type = 'temporary';
  }
  ...
  return (
    <div className={`app-sidebar d-none ${drawerStyle}`}>
      <Drawer className="app-sidebar-content"
                variant={type}
                open={type.includes('temporary') ? navCollapsednew : true}
